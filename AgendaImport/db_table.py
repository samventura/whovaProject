...
class db_table:
    ...
    def create_table(self):
        ...
        # Modified .iteritems() to .items() for python3 functionality (using python3)
        columns_query_string = ', '.join(
            ["%s %s" % (k, v) for k, v in self.schema.items()])

        # print("Query string: %s" % columns_query_string)
    ...
    def select(self, columns=[], where={}):
        ...
        if where:
            where_query_string = ["%s = '%s'" %
                                  (k, v) for k, v in where.items()]  # Changed to .items() for python3
            query += " WHERE " + ' AND '.join(where_query_string)
            # Added to get particular speakers by name
            if "speakers" in where:
                query += (' OR speakers LIKE \'%' + where["speakers"] + '%\'')
